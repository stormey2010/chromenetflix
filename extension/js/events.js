...
const ncEvents = {
  ...
  async sendSyncCommand(command, time) {
    ...
    // Don't send sync if video is in an invalid state
    const video = ncGetVideo();
    if (!video || video.readyState < 2) {
      console.log('[Netflix Connect] Skipping sync - video not ready');
      return;
    }
  ...
  init() {
    ...
    // Also track visibilitychange for tab switching edge cases
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden') {
        // Don't immediately disable, but set a flag to be more cautious
