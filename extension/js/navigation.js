...
const ncNavigation = {
  ...
  checkUrlChange() {
    if (window.location.href !== this.lastUrl) {
      ...
      // Track recently watched when starting to watch something
      if (ncGetPageType() === 'watch') {
        this.trackRecentlyWatched();
      }
    }
  },
  
  // Track recently watched content
  async trackRecentlyWatched() {
    if (!ncUser.current || ncUser.current === 'unknown') return;
    ...
    const episodeEl = document.querySelector('[data-uia="video-title"] + span');
    if (episodeEl) {
      const match = episodeEl.textContent.match(/S(\d+):E(\d+)/i);
      if (match) {
        season = parseInt(match[1]);
        episode = parseInt(match[2]);
      }
      episodeTitle = episodeEl.textContent.replace(/S\d+:E\d+\s*/, '').trim();
    }
    
    // Recently watched tracking removed from server
    console.log('[Netflix Connect] Navigation tracked (recently watched disabled):', title);
  },
