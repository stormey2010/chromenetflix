...
const ncNotifications = {
  ...
  showShareReceived(from, title, url) {
    return this.show({
      ...
      duration: 0,
      actions: [
        { label: 'Details', primary: true, action: () => { if (url) window.location.href = url; } },
  ...
  pendingAccept: false,  // Flag to prevent duplicate connected notification
  
  async acceptInvite(from) {
    try {
      this.pendingAccept = true;  // Mark that we initiated this
      ...
      // Reset flag after a short delay to allow SSE event to be ignored
  ...
  handleEvent(data) {
    ...
    if (data.event === 'invite_received' && data.to === me) this.showInviteReceived(data.from);
      ...
      // Skip if we just accepted (already showed notification)
